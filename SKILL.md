...
| `<!-- boucle:diagram-unfit v=1 kinds=<kinds> types=<types> rounds=<n> -->` | `v=1 kinds=<closed-set> types=<Mermaid-block-types> rounds=<n>` or `v=1 state=exhausted rounds=<n>` | gates.sh (check_diagram_fit_gate) | gates.sh (retry-bound count) | Marks a spec whose diagram parses but does not draw what the spec changes — a block type no declared impact kind is drawn with, or an `erDiagram` whose entities are the spec's own `## Impacted files`. CI re-triggers triage to redraw ONLY the diagram. After `BOUCLE_DIAGRAM_FIT_MAX_RETRIES` rounds the marker carries `state=exhausted`. |
...
| `<!-- boucle:diagram-invalid v=1 rounds=... -->` | `v=1 rounds=<n>` (+ `state=exhausted` once the bound is spent) | gates.sh (check_diagram_syntax_gate) | gates.sh (counts its own markers for the retry bound) | Diagram SYNTAX verdict (LESSONS.yml lesson #108). `bin/check-mermaid` runs every ```mermaid fence in the spec through the real Mermaid parser; a parse error means the forge renders an error box instead of a diagram, so the gate blocks spec-review and re-triggers triage with the parser's own message. Fail-open when the parser is unavailable (no node, no modules, no network) — infrastructure never blocks the loop. |
| `<!-- boucle:diagram-unfit v=1 kinds=... types=... rounds=... -->` | `v=1 kinds=<closed-set> types=<Mermaid-block-types> rounds=<n>` (+ `state=exhausted` once the bound is spent) | gates.sh (check_diagram_fit_gate) | gates.sh (counts its own markers for the retry bound) | Diagram FIT verdict (LESSONS.yml lesson #108). Cross-checks the `## Diagram` section against the spec's own `## Impacts` and `## Impacted files` markers: (a) every declared block type must be one its declared impact kinds are drawn with, (b) an `erDiagram` whose entities are the spec's own impacted files is the file wiring, not a data model. Fail-open on a missing marker, an unknown impact kind, or `kinds=none`. |

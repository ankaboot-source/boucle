...
47. **Anchor log-scraping awk marker patterns to start-of-line**
    - ❌ DO NOT use an unanchored awk regex like
      ...
      **meta-instructions to itself** that quote the marker in prose, e.g.
      ...
      role=reviewer sha=abc -->\` marker.` An unanchored `$0 ~` matches the
      marker substring inside that prose sentence, sets `found=1`, and
...
49. **Preserve instructed content**
    - ❌ DO NOT generate placeholders for content the issue instructs. If
      ...
      placeholder in place of the instructed URL, never a paraphrase,
      never a substitute.
    ...
    - ❌ DO NOT let later amendments override earlier preservation
      ...
      amendments (e.g. "fill empty spaces with a pattern", "single CTA")
      ...
      and degrades the validated layout — replacing the video with the
      pattern instead of filling the empty space below it.

...
- `AGENTS.md` — agent workflow rules, mandatory principles. **The lessons that matter for this issue are already in your prompt** (extracted from both `LESSONS.yml` at the repo root — this repository's own — and the engine's under `.boucle/`). Read them there. Only open a file directly if you need a lesson the extraction did not surface.
...
7. **Write the acceptance criteria** — verifiable by a machine or by looking at the rendered page, using the Given/When/Then format (§2) with Happy path / Edge case / Error state / Non-functional labels. These are **proposed** criteria (the human approves them next), but do not label the section "Draft" — the section is `### Acceptance` under `## Criteria`.

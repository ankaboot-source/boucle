...
3. Read the acceptance criteria from `state.md` — knowing they were **frozen at triage time** (see next step).
   ...
   **Also read `## Non-goals` and check every one.** Criteria say what must become true; non-goals say what must stay false. A diff that satisfies all criteria while crossing a non-goal is a **FAIL** — list the violated non-goal as an unmet criterion in your verdict. This is the only thing standing between "satisfies the spec" and "did something nobody asked for", so do not treat it as advisory. A human comment may lift a non-goal, exactly as it may amend a criterion.
4. **Read the "Prior MR discussion" section of your prompt** (if present). It contains previous reviewer verdicts and human comments on the MR. Human comments **AMEND the spec mid-loop**: when a human comment contradicts or refines a frozen acceptance criterion from `state.md` (or the issue body), **the human comment wins**. Grade the implementation against the amended spec, not the frozen triage spec — a worker that correctly implements a human's amendment MUST NOT be FAILed for diverging from the triage-era criterion. Previous reviewer verdicts are context, not authority: re-verify against the current (amended) spec.
   ...
   **Creative proposals are NOT acceptance criteria.** The triage's `## Creative proposals` and `## Consequences` sections are advisory — they expand the solution space but the worker is NOT bound by them. You MUST NOT turn a creative proposal into a 🔴 FAIL criterion. Only the `### Acceptance` and `### Must-haves` blocks of the triage's `## Criteria` section — mirrored into `state.md` as `## Acceptance criteria` and `## Must-haves`, and as amended by human comments — are binding. If a creative proposal was meant to be a deliverable, the triage should have put it in the acceptance criteria — its absence from `## Creative proposals` is a triage gap, not a worker failure.

   **Identifying human vs boucle notes (marker, never identity).** Every injected note is tagged `[<author> — human]` or `[<author> — boucle]`; CI computes the tag from the `<!-- boucle:agent -->` marker, and the tag is the ONLY authorship signal. In mono-user mode boucle posts under the human's own account, so a `— human` note may come from the same account that posts all bot verdicts — that is expected, not evidence of a bot comment. NEVER reclassify a `— human` note as bot-authored because of its account, its tone, or because it contradicts the issue body or state.md: a human amending their own spec (even against the frozen wording) is the amendment channel working as designed, and the newest human instruction wins.
   ...
   **Re-check EVERY acceptance criterion on every iteration — never only the ones you failed last time.** A criterion that passed on iteration N-1 can be broken by the very fix that addressed the failing one. Grading only the previously-unmet criteria is how a regression introduced by the fix reaches production with a PASS on it.
   ...
   A prior verdict reaches you reduced to its unmet criteria, with the reasoning removed. That is deliberate and not configurable: it stops you ratifying your own earlier reasoning instead of re-verifying. Human comments are never filtered — they always reach you in full, and they always win.
5. **Test the deployed preview URL** (provided in `$BOUCLE_PREVIEW_URL`), NOT a local build. **If `$BOUCLE_PREVIEW_URL` is EMPTY** (diff review mode — e.g. GitLab Pages declarative deployment with no preview), grade against `$BOUCLE_MR_DIFF` (the MR diff, injected by CI) and `$BOUCLE_MR_CHECKS` (the check suites) instead: verify each criterion against the code in the diff, not a deployed site, and note in your verdict that this was a diff review (no preview deploy). If CI injected a `Pre-existing CI failures` block, those check suites also fail on the merge base — they are **NOT regressions introduced by this MR**. Do NOT FAIL any acceptance criterion on their account; note them as pre-existing in your verdict so the human knows the default branch is already red and can fix it separately.
...
10. **If your prompt carries a "Functional navigation checks" section, start there.** A real browser drove the preview before you were invoked; each `NAV:` line is a criterion that was clicked through, not inferred from HTML. Those lines are established facts — never re-verify them with `curl`. Every `NAV: FAIL` is a blocking `- [ ] 🔴` item and makes the verdict FAIL; a TIMEOUT or "no assertion ran" is UNCERTAIN, never PASS. Then fetch the preview URL with `curl` and verify the HTML contains expected content for the criteria the browser did NOT cover. **Batch your checks**: fetch each page ONCE and grep for all relevant patterns in that single response — do NOT re-fetch the same page for every criterion. Prefer a single `curl -s <url> | grep -E 'pattern1|pattern2|pattern3'` over many sequential `curl` calls.
...
Post your **final verdict** as a comment on the MR/PR (use `bin/forge-note mr <mr_iid> --message "..."`) with this format:
...
4. **Not already covered** — not a restatement of an existing lesson in either `LESSONS.yml`?
5. **The right file** — repo-specific entries go in `LESSONS.yml` at the repo root (validated with `bin/check-lessons LESSONS.yml --against .boucle/LESSONS.yml`); a universal one is NEVER committed to `.boucle/` — it is stated on stdout for upstreaming. In the engine repo there is one file and this does not apply.

...
### 0.C If the brief is ambiguous, derive a single best read

**CI mode:** no human is available to clarify. Do not ask a clarifying question. Pick the most plausible design read from the brief's vibe words, reference signals, audience, and brand assets; if multiple reads are equally likely, default to the conservative one (Linear-clean for B2B, editorial for portfolio, premium-consumer for DTC). Declare the assumption in the "Design Read" one-liner and proceed. The result is auditable; a reviewer who disagrees can re-run with a sharper brief.
...
**Baseline:** `8 / 6 / 4`. Use these unless the design read overrides them. In CI mode, derive dial overrides from the brief and the Section 1.A inference table; do not wait for a human to edit this file.
...
3. **Last resort: leave placeholder slots.** If neither is possible, do NOT fill the page with hand-rolled SVG illustrations or div-based "fake screenshots." Instead, leave clearly-labeled placeholder slots (`<!-- TODO: hero product photo, 1600x1200 -->`) and at the end of the response output a list of placements that need real images so the next session knows what to generate.
...
If ambiguous, default to **Redesign - Preserve**: the safer default. Document the choice in the audit and proceed; the assumption can be reversed in a follow-up redesign pass if the brief later clarifies.
...
In **CI mode** (no human approval available), these are **hard rules, not review gates** — they must be preserved by default. If the brief explicitly calls for changing one, the agent may change it, but must record the change in the audit log alongside the rationale:
...
npx -y shadcn@latest add button card badge separator input

...
boucle_ci_e2e() {
    ...
    # Symmetric to maybe_close_parent.
    maybe_unblock_dependents() {
        ...
        if [ -z "$sibling_iids" ]; then
            ...
            if [ -z "$sibling_iids" ]; then
                local links_data
                # TODO: forge_* — no issue-links endpoint in the forge contract yet.
        ...
        for sib_iid in $(echo "$sibling_iids" | tr ',' ' '); do
            ...
            for dep_iid in $(echo "$sib_deps" | tr ',' ' '); do
                dep_state=$(forge_issue_get "$dep_iid" 2>/dev/null | jq -r '.state // "unknown"' 2>/dev/null || echo "unknown")
    ...
    # yet exit non-zero (e.g. jcode cleanup, glab "new version" warning on
    ...
    # glab (API error), or jq (bad JSON) would kill the script before the
    ...
    # Tolerate glab/jq non-zero exit (glab may warn about version updates on

...
HELP_EOF
  ...
  # before adopting a disposition, and set INCOMPLETE_SPEC=1 when it fails
  # so the no-disposition handler escalates instead of routing an empty
  # spec. A draft that legitimately lacks ## TL;DR is left in place for
  # the agent (or a later run) to refine — it is never promoted to a
  # routing decision.
  comment_has_tldr() {
  ...
  if [ -n "$PRE_DRAFT_NOTE_ID" ]; then
    ...
    if [ -n "$PRE_DRAFTED_COMMENT" ]; then
      # Spec-completeness guard (lesson #92): a draft without ## TL;DR
      # is an incomplete spec — do NOT promote it to a routing
      # decision. Leave it in place for the agent to refine.
      if ! comment_has_tldr "$PRE_DRAFTED_COMMENT"; then
        echo "[boucle] Pre-agent: draft (note $PRE_DRAFT_NOTE_ID) has no ## TL;DR — incomplete spec, NOT promoting (lesson #92)."
      else
  ...
  if [ -z "$DISPOSITION" ]; then
    ...
    if [ -n "$DRAFT_NOTE_ID" ]; then
      DRAFTED_COMMENT=$(forge_issue_note_get "$IID" "$DRAFT_NOTE_ID" 2> /dev/null \
        | jq -r '.body // empty' 2> /dev/null)
      if [ -n "$DRAFTED_COMMENT" ]; then
        # Spec-completeness guard (lesson #92): a posted draft without
  ...
  case "$DISPOSITION" in
    READY)
      if [ "$SIZE" = "L" ]; then
        ...
        # which is the inference-on-agent-output trap LESSONS.yml lesson #87
  ...
  if [ "${BOUCLE_PREVIEW_DISABLE:-false}" != "true" ] && [ -s "$RENDER_REQUEST_FILE" ] && [ -s "$PREVIEW_HTML" ]; then
    ...
    if [ -z "$TRIAGE_NOTE_ID" ]; then
      ...
      # 2. Render preview.html → one PNG per viewport.
      #    bin/render-preview drives agent-browser against the Chromium baked
      #    into the agents image (or the runner's own), so nothing is
      #    downloaded here. The former puppeteer-core + @sparticuz/chromium
      #    install cost 103 MB from npm plus a 191 MB extraction into /tmp,
      #    9.2 s before the first pixel on a cold cache.
      ...
      #    render errors.
      RENDERED_PNGS=$("$BOUCLE_HOME/bin/render-preview" "$PREVIEW_HTML" "$PREVIEW_PNG" 2> "$RENDER_STDERR" || true)
...
"
        ...
        echo "[boucle] WARN: preview render failed — posting fallback note"

...
@test "disabled by default (no BOUCLE_DND_ENABLED) → exit 1" {
  run env -u BOUCLE_DND_ENABLED BOUCLE_DND_NOW=0 bin/dnd
  assert_failure
}

@test "BOUCLE_DND_ENABLED=false → exit 1 even inside window" {
  run env BOUCLE_DND_ENABLED=false BOUCLE_DND_NOW=1700000000 bin/dnd
  assert_failure
}

@test "BOUCLE_DND_ENABLED=true with no other config uses defaults (22:00→07:00 UTC)" {
  # 22:20 UTC on 2023-11-14 = epoch 1700000400 → inside 22:00-07:00
  run env BOUCLE_DND_ENABLED=true BOUCLE_DND_NOW=1700000400 bin/dnd
  assert_success
  # 12:00 UTC on 2023-11-14 = epoch 1699963200 → outside
  run env BOUCLE_DND_ENABLED=true BOUCLE_DND_NOW=1699963200 bin/dnd
  assert_failure
}

# ── Overnight wrap (default 22:00 → 07:00) ───────────────────────────

@test "22:30 UTC → inside overnight window" {
  run env BOUCLE_DND_ENABLED=true BOUCLE_DND_NOW=1700001000 bin/dnd # ~22:30 UTC
  assert_success
}

@test "02:00 UTC → inside overnight window (after midnight)" {
  # 2023-11-14T02:00:00Z = 1699927200
  run env BOUCLE_DND_ENABLED=true BOUCLE_DND_NOW=1699927200 bin/dnd
  assert_success
}

@test "07:30 UTC → outside overnight window (after END)" {
  # 2023-11-14T07:30:00Z = 1699947000
  run env BOUCLE_DND_ENABLED=true BOUCLE_DND_NOW=1699947000 bin/dnd
  assert_failure
}

@test "21:59 UTC → outside overnight window (before START)" {
  # 2023-11-14T21:59:00Z = 1699999140
  run env BOUCLE_DND_ENABLED=true BOUCLE_DND_NOW=1699999140 bin/dnd
  assert_failure
}

@test "boundary: exactly 22:00 → inside (inclusive START)" {
  # 2023-11-14T22:00:00Z = 1699999200
  run env BOUCLE_DND_ENABLED=true BOUCLE_DND_NOW=1699999200 bin/dnd
  assert_success
}

@test "boundary: exactly 07:00 → outside (exclusive END)" {
  # 2023-11-14T07:00:00Z = 1699945200
  run env BOUCLE_DND_ENABLED=true BOUCLE_DND_NOW=1699945200 bin/dnd
  assert_failure
}
...
@test "same-day window 09:00→17:00: 12:00 inside" {
  run env BOUCLE_DND_ENABLED=true BOUCLE_DND_START=09:00 BOUCLE_DND_END=17:00 \
...
@test "degenerate zero-length window (START==END) → never DND" {
  run env BOUCLE_DND_ENABLED=true BOUCLE_DND_START=12:00 BOUCLE_DND_END=12:00 \
...
@test "excluded day suppresses DND even inside window" {
  ...
  run env BOUCLE_DND_ENABLED=true BOUCLE_DND_NOW=1700262000 \
    BOUCLE_DND_EXCLUDE_DAYS=Fri bin/dnd
  assert_failure
}

@test "non-excluded day still allows DND" {
  # 2023-11-14 is a Tuesday. 23:00 UTC Tuesday → inside, Tue not excluded.
  run env BOUCLE_DND_ENABLED=true BOUCLE_DND_NOW=1700000400 \
...
@test "TZ shifts the window relative to UTC epoch" {
  ...
  # the script runs without error when a TZ is set and the default UTC
  # path still works.

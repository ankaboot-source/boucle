...
@test "health: boucle_health_record appends a JSONL line with the right fields" {
  ...
  assert_success
  run jq -r 'select(.role=="worker") | .role + ":" + (.iteration|tostring) + ":" + (.exit_code|tostring) + ":" + (.prompt_chars|tostring) + ":" + .tokens + ":" + .cost_usd + ":" + .model + ":" + .provider' "$T/.boucle/7/health.jsonl"
  assert_output "worker:2:0:1500:400:0.01:deepseek-v4-flash:boucle"
  run jq -r 'select(.role=="worker") | .timestamp' "$T/.boucle/7/health.jsonl"
...
@test "health: boucle_health_outcome appends an outcome line" {
  ...
  assert_success
  run jq -r 'select(has("outcome")) | .role + ":" + .outcome + ":" + .detail' "$T/.boucle/7/health.jsonl"
...
@test "health: diagnostic classifies a no-changes trigger as step-budget-exhaustion" {
  ...
  cat > "$T/.boucle/7/health.jsonl" << 'JSONL'
...
@test "health: diagnostic classifies an exit-4 trigger as provider/quota" {
  ...
  mkdir -p "$T/.boucle/7"
  health_funcs "$TMPF"
  printf '%s\n' '{"role":"worker","outcome":"no-changes"}' > "$T/.boucle/7/health.jsonl"
...
@test "health: diagnostic embeds the cost summary when cost.json exists" {
  ...
  health_funcs "$TMPF"
  printf '%s\n' '{"role":"worker","outcome":"no-changes"}' > "$T/.boucle/7/health.jsonl"
  echo '{"entries":[{"role":"worker","iteration":1,"prompt_tokens":100,"completion_tokens":10,"cost_usd":0.01}]}' > "$T/.boucle/7/cost.json"
  run bash -c "BOUCLE_WORKSPACE='$T'; source '$TMPF'; boucle_escalation_diagnostic 7 no-changes"
  assert_success
